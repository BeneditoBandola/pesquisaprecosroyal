...
def gerar_pdf_relatorio(promotor, loja, cidade, df_preenchido, df_vendas_original):
    ...
    def limpar_valor(v):
        if pd.isna(v) or v == "" or str(v).lower() == "none":
            return 0.0
...
if not vendas.empty:
    if st.session_state.promotor_logado is None:
        ...
        if c1.button("👩‍💼 PAMELA", use_container_width=True):
            st.session_state.promotor_logado = "Pamela"
            st.rerun()
        if c2.button("👩‍💼 FERNANDA", use_container_width=True):
            st.session_state.promotor_logado = "Fernanda"
            st.rerun()
        if c3.button("👩‍💼 MADALLA", use_container_width=True):
            st.session_state.promotor_logado = "Madalla"
            st.rerun()
    else:
